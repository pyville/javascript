...
// 버튼(job, mail, birthday)에 적용할 패턴
const patternsByFields = {
    job: /.+[사]/,
    mail: /[a-zA-Z0-9._+-]+[@][a-zA-Z]+[(.net)]/,
    birthday: /1996/
}

// 정규 표현식에 따라 필터링하고, 객체의 이름(name) 속성으로 매핑 
const reg = (field) => {
    return profileArr
...
// 모든 버튼에 대해 forEach 문을 적용
buttons.forEach((button)=>{
    //e.target.id로 버튼의 id를 받아서 reg 함수 실행하고, 그 값을 resultDiv에 표시
